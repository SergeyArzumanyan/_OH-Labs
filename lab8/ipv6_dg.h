/*
 * ipv6_dg.h
 * Header file for IPv6 datagram socket programs
 * 
 * Defines:
 * - BUF_SIZE: Maximum size of messages exchanged between client and server
 * - PORT_NUM: Server port number
 * - errExit(): Error handling function
 * 
 * Includes necessary headers for IPv6 socket programming
 */

#include <stdio.h>      // Standard I/O functions (fprintf, stderr)
#include <stdlib.h>     // Standard library functions (exit, EXIT_FAILURE)
#include <netinet/in.h> // Internet address structures (struct sockaddr_in6)
